...
SUBROUTINE start
  ...
  DO c=1,chunks_per_task
    ...
    x_cells = right(chunk_task_responsible_for) -left(chunk_task_responsible_for)  +1
    y_cells = top(chunk_task_responsible_for)   -bottom(chunk_task_responsible_for)+1
      
    IF(chunks(c)%task.EQ.parallel%task)THEN
      CALL build_field(c,x_cells,y_cells)
    ENDIF
    chunks(c)%field%left    = left(chunk_task_responsible_for)
    chunks(c)%field%bottom  = bottom(chunk_task_responsible_for)
    chunks(c)%field%right   = right(chunk_task_responsible_for)
    chunks(c)%field%top     = top(chunk_task_responsible_for)
    ...
    chunks(c)%field%x_max = right(chunk_task_responsible_for)-left(chunk_task_responsible_for)+1
    chunks(c)%field%y_max = top(chunk_task_responsible_for)-bottom(chunk_task_responsible_for)+1
